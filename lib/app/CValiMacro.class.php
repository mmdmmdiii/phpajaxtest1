...
class CValiMacro {
	...
	public static function isValidColor( $v ) {
		return ( preg_match('/^\#[0-9abcdefABCDEF]{6}$/', $v) == true );
	}
	...
	public static function vColor( $key, $b_required = true, $b_assign = true ) {
		if ( trim(self::$fm[$key]) == "" ) {
			if ( $b_required ) {
				CFRes::setES($key);
				CFRes::addEA(self::$lca["err:empty:{$key}"]);
			}
		} else if ( !self::isValidColor(self::$fm[$key]) ) {
			CFRes::setES($key);
			CFRes::addEA(self::$lca["err:invalid:{$key}"]);
		}
		if ( $b_assign ) {
